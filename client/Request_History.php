			...
			<table class="table table-hover table-striped" align = "center" style = "width: 65%;" >
				...
				<?php
					...
					while($myrow = mysqli_fetch_array($result))
					{
						...
						echo '<td>';
						echo 	'<div>';
						echo 		'<button class="btn btn-success btn-xs" onclick = "view('.$x.');" data-toggle="modal" data-target=".bd-example-modal-sm" id ="'. $x .'.view">view</button>';
						echo 	'</div>';
						echo '</td>';
						echo '</tr>';
						$x++;
...
<!--Initialization of needed bootstrap-select files-->		
	...
	<script>
		...
		function view(i)
		{
			table_new = table_string;
			var ris_no = document.getElementById(i+".ris").innerHTML;
			//alert("goes here");
			$.ajax({
				type: 'POST',
				url: 'View_RIS.php',
				...
				success: function(response){
					var number_of_records = response["total_number"];
					var i = 0;
					for(i = 0; i < number_of_records; i++)
					{
						 table_new = table_new +
										'<tr>'+
										'<td>'+ response["stock_no"+i] +'</td>'+
										'<td>'+ response["unit"+i] +'</td>'+
										'<td>'+ response["description"+i] +'</td>'+
										'<td>'+ response["quantity"+i] +'</td>'+
										'</tr>';
						
						
					}
					table_new = table_new + '</table>';
					document.getElementById("table_view_body").innerHTML = table_new;
				},
